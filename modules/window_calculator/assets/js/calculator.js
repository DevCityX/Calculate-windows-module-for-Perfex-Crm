(function () {
  function boot() {
    ...
    if (!controls.profile || !controls.shape || !controls.svgWrap) {
      return;
    }
    ...
    function initSections() {
      if (!controls.sectionsGrid) return;
    ...
    function recalc() {
      ...
      if (!profile) {
        if (controls.breakdown) controls.breakdown.innerHTML = '<div class="text-danger">Немає товарів для розрахунку.</div>';
        return;
      }
      ...
      if (controls.svgWrap) controls.svgWrap.innerHTML = svg;
      if (controls.svgWrap2) controls.svgWrap2.innerHTML = svg;
      if (controls.breakdown) {
    ...
    function saveVisual() {
      ...
      const proposalId = parseInt((controls.proposalId && controls.proposalId.value) || '0', 10);
      if (proposalId < 1) return notify('danger', 'Вкажіть ID пропозиції.');

      const data = {
        proposal_id: proposalId,
        title: (controls.title && controls.title.value) || 'Віконна конструкція',
      ...
      if (window.WINDOW_CALCULATOR_BOOTSTRAP && window.WINDOW_CALCULATOR_BOOTSTRAP.csrfName) {
        data[window.WINDOW_CALCULATOR_BOOTSTRAP.csrfName] = window.WINDOW_CALCULATOR_BOOTSTRAP.csrfHash;
      }

      if (!window.jQuery || !window.WINDOW_CALCULATOR_BOOTSTRAP || !window.WINDOW_CALCULATOR_BOOTSTRAP.saveUrl) {
        notify('danger', 'Не вдалося ініціалізувати збереження.');
        return;
      }
      ...
      $.post(window.WINDOW_CALCULATOR_BOOTSTRAP.saveUrl, data)
        .done(function (res) {
          ...
          if (r && r.csrf_hash && window.WINDOW_CALCULATOR_BOOTSTRAP.csrfName) {
            window.WINDOW_CALCULATOR_BOOTSTRAP.csrfHash = r.csrf_hash;
          }
    ...
    if (controls.calcBtn) controls.calcBtn.addEventListener('click', recalc);
    if (controls.saveBtn) controls.saveBtn.addEventListener('click', saveVisual);
